...
```{r}
cleaned_project_list <- readRDS("cleaned_project_list.rds")
project_names <- readRDS("project_names.rds")
user_names <- readRDS("user_names.rds")
```
...
```{r}
popular_projects <- read_csv("popular_projects.csv")
popular_projects$n_watchers <- as.integer(popular_projects$n_watchers)
summary(popular_projects)
```
...
project_life %>% 
  ggplot(aes(num_weeks,fill=group)) +
...
```{r}
# standardize data and remove outlier
scale_phase <- function(phase_commits){
  scaled_phase <- data.frame(scale(phase_commits[,1:3]))
  standard_phase <- scaled_phase %>% 
    subset(anaphase < 1 & anaphase > -1) %>% 
    subset(metaphase < 1 & metaphase > -1) %>% 
    subset(prophase < 1 & prophase > -1)
...
```{r}
# small data points
scale_phase_small <- function(phase_commits){
  scaled_phase <- data.frame(scale(phase_commits[,1:3]))
  standard_phase <- scaled_phase %>% 
    subset(prophase < 0 & metaphase < 0 & anaphase < 0)
  return(standard_phase)
}
standard_small_100 <- scale_phase_small(phase_commits_100)
standard_small_75 <- scale_phase_small(phase_commits_75)
standard_small_50 <- scale_phase_small(phase_commits_50)
standard_small_25 <- scale_phase_small(phase_commits_25)
```
